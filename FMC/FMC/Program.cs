...
app.Use(async (context, next) =>
{
    ...
    Console.WriteLine($"[MAINT-MW] Request: path={path} IsActive={FMC.Services.MaintenanceState.IsActive} User={context.User.Identity?.Name} Roles={string.Join(",", context.User.Claims.Where(c=>c.Type==System.Security.Claims.ClaimTypes.Role).Select(c=>c.Value))}");
